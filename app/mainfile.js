            ...
            this.state.preview ?
                <View style={styles.container}>
                    <Image
                        ...
                        style={{ flex: 1 }}
                    />
                    <View style={{ flex: 0.25, flexDirection: 'row-reverse', justifyContent: 'space-around', alignItems: 'center', backgroundColor: 'grey' }}>
                ...
                <View style={styles.container}>
                    {this.state.Camera === false ?
                        <View style={styles.gallery}>
                            {this.state.images.length === 0 ? <Text>Empty...</Text> :
                                <View style={{ alignItems: 'center', flex: 1, flexDirection: 'column' }}>
                                    <View style={{ flex: 0.1 }}>
                                        <Text style={{ textAlign: 'center' }}>Click on images to open, edit &amp; delete.</Text>
                                    </View>
                                    <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'flex-start', flexWrap: 'wrap' }}>
                                        {this.state.images.map((e, i) => {
                                            return (
                                                <TouchableOpacity onPress={() => { this.setState({ currentPrivewId: i, preview: true }) }} key={i} style={{ overflow: 'hidden', backgroundColor: 'grey', width: 100, height: 100, borderWidth: 2, margin: 2, borderRadius: 0 }}>
                                                    <Image
                                                        key={i}
                                                        source={{ uri: e }}
